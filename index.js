...
function addGamesToPage(games) {
    ...
    for (let i = 0; i < games.length; i++){
        ...
        game_card.innerHTML = `<img class="game-img" src="${games[i].img}"></img> <h1>${games[i].name}</h1> <p>${games[i].description}</p> <p>Backers: ${games[i].backers}</p>`;
...
Currently, ${listOfUnfunded.length} ${listOfUnfunded.length > 1 ? "games remain" : "game remains"}
unfunded. We need your help to fund these amazing games!`

// create a new DOM element containing the template string and append it to the description container
let unfundedText = document.createElement("p");
unfundedText.innerHTML = `${displayStr}`;
...
console.log(sortedGames[i].name);
}
